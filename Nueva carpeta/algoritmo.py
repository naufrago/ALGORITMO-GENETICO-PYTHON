...
# ND: Cuantos individuos se seleccionan para reproduccion. Necesariamente
# mayor que 2
padres = 0
...
class Ui_Dialog(object):
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(
            _translate("Dialog", "EVALUACION ALGORITMO GENETICO"))
        self.label.setText(_translate("Dialog", "ALGORITMOS GENETICOS"))
        self.lind.setText(_translate("Dialog", "0"))
        self.label_2.setText(
            _translate("Dialog", "LIND: \"limite de bits que tendra cada individuo\""))
        self.label_3.setText(_translate(
            "Dialog", "ELEMENTO DEL MODELO ALCANZAR NUMEROS ADMITIDOS DEL 0 AL 9"))
        self.asignar.setText(_translate("Dialog", "ASIGNAR"))
        self.label_4.setText(
            _translate("Dialog", "TAMAÑO DE POBLACIÓN : (>=5)"))
        self.label_5.setText(
            _translate("Dialog", "# DE PADRES A SELECCIONAR :  (>=3)"))
        self.label_6.setText(
            _translate("Dialog", "PROBABILIDAD DE MUTACIÓN: (%)"))
        self.label_7.setText(_translate("Dialog", "ITERACIONES DESEADAS"))
        self.label_8.setText(
            _translate("Dialog", "SELECCIONAR EL NUMERO DE ITERACIONES DESEADAS"))
        self.label_9.setText(
            _translate("Dialog", "PARA LA BUSQUEDA DEL MODELO, O DEJAR  EN CERO "))
        self.label_11.setText(
            _translate("Dialog", "PARA  ITERAR 10000 VECES."))
        ...
        self.label_10.setText(
            _translate("Dialog", "SOLUCIÓN   HALLADA  SEGUN LAS ITERACIONES DADAS"))
        ...
        self.label_13.setText(
            _translate("Dialog", "recomendable minimo 3 bits."))
    ...
    def imprimir(self):
        ...
        if self.npadres.text() == "" or int(self.npadres.text()) <= 2 or int(self.npadres.text()) >= int(self.tpoblacion.text()):
            ...
            evaluar = False
            mensaje = mensaje + \
                "*CAMPO #DE PADRES vacio o es un numero < 3 o el numero de padres es >= a la poblacion,  \n"
        ...
        if evaluar:
            ...
            detalle1 = detalle1+"*MODELO INGRESADO POR EL USUARIO: " + \
                str(modelos) + "\n"
            detalle1 = detalle1+"*LIMITE DEL INDIVIDUO : "+str(lInd) + "\n"
            detalle1 = detalle1+"*POBLACION DE TAMAÑO : "+str(tamano) + "\n"
            detalle1 = detalle1 + \
                "*CANTIDAD DE MEJORES PADRES : "+str(padres) + "\n"
            detalle1 = detalle1+"*SE REALIZARON : " + \
                str(iteraciones) + " ITERACIONES\n\n"
            ...
            for x in range(numItera):  # numero de iteraciones para evaluar
                ...
                for y in range(len(listap)):  # recorre la lista de poblacion
                    # toma la lista un individuo con sus bits
                    listax = listap[y]
                    fitness = 0
                    # recorre la  lista de  bits del individuo seleccionado
                    for a in range(len(listax)):
                        ...
                        if lis == mod:
                            fitness += 1
                    # guarda los pares ordenados (#de fitnes hallados, lista de
                    # bits del individuo)
                    puntuados.append((fitness, listax))
                #print("Poblacion seleccionada:\n%s"%(puntuados))
                detalles2 = detalles2 + \
                    "* POBLACION SELECCIONADA EN PARES ORDENADOS  (FITNESS, INDIVIDUO):\n"+str(
                        puntuados)+"\n\n"
                ...
                for i in range(len(poblacion)-padres):
                    ...
                    #print("padres \n%s"%(padre))
                    # Se mezcla el material genetico de los padres en cada
                    # nuevo individuo
                ...
                for s in range(len(poblacion)):  # cantidad de individuos
                    # cantidad de bits de cada individuo
                    for q in range(len(poblacion[s])):
                        sumatoria = sumatoria+int(poblacion[s][q])

                xx.append(int(x+1))
                # almacena en el vector plot el resultado de la suma
                yy.append(sumatoria)
    ...
    def val_grafica(self):
        ...
        pyplot.title(
            "comportamiento  de la sumatoria de los bit del los individuos\nde la poblacion hallada  en cada iteracion")
        pyplot.xlabel("iteraciones")   # Establece el título del eje x
        # Establece el título del eje y
        pyplot.ylabel("sumatoria de cada poblacion")
        pyplot.show()
    ...
    def validar_pmutacion(self):
        ...
        if pmutacion == "":
            ...
            if validar and p > 1.0:
                ...
                ctypes.windll.user32.MessageBoxW(
                    0, "LA PROBABILIDAD DEBE ESTAR ENTRE 0 Y 1", "ALERTA", 0)
                return False
            else:
    ...
    def reiniciar(self):
        ...
        ctypes.windll.user32.MessageBoxW(
            0, "< SE REINICIARA TODA LA APLICACION >", "ALERTA", 0)
        m = len(modelos)
        d = len(detalles)
        for x in range(m):
            modelos.pop()
    ...
    def mostar_modelo(self):
        if (self.lind.text() != "0" and self.lind.text() != "" and self.lind.text() != "1" and self.lind.text() != "2" and self.lind.text() != "01" and self.lind.text() != "02") and self.modelo.text() != "":
            ...
            if bits > 0:
                ...
                ctypes.windll.user32.MessageBoxW(
                    0, "INGRESE EL SIGUIENTE BIT", "ALERTA", 0)
                ...
                ctypes.windll.user32.MessageBoxW(
                    0, "YA INGRESO TODOS LOS BITS REQUERIDOS", "ALERTA", 0)
        else:
            men = ""
